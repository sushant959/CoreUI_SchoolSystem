...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `_billing1` AS (select `dbo`.`tblreceipt`.`StudentId` AS `StudentId`,min(`dbo`.`tblreceipt`.`DueAmount`) AS `Due` from `dbo`.`tblreceipt` where (`dbo`.`tblreceipt`.`Month` = '1') group by `dbo`.`tblreceipt`.`StudentId`) */;
